...
static uint32_t g_master_tx_buff[BUFF_LEN] __attribute__((section(".noncache_buffer")));   // Master Transmit Buffer
static uint32_t g_master_rx_buff[BUFF_LEN] __attribute__((section(".noncache_buffer")));   // Master Receive Buffer
static uint32_t g_slave_tx_buff[BUFF_LEN] __attribute__((section(".noncache_buffer")));    // Slave Transmit Buffer
static uint32_t g_slave_rx_buff[BUFF_LEN] __attribute__((section(".noncache_buffer")));    // Slave Receive Buffer
...
fsp_err_t spi_init(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_SPI_B_Open API for SPI Master failed ** \r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        if ( (FSP_SUCCESS != R_SPI_B_Close(&g_spi_master_ctrl)))
        {
            ...
            APP_ERR_PRINT("** R_SPI_B_Close API for SPI Master failed ** \r\n");
        }
...
/*******************************************************************************************************************//**
 * @brief This function demos both R_SPI_B_Write() and R_SPI_B_Read() individually.
 * @param[IN]   None
...
fsp_err_t spi_write_and_read(void)
{
    ...
    APP_PRINT("\r\nMASTER SEND: %s",g_master_tx_buff);
    ...
    APP_PRINT("\r\nSLAVE RECEIVE: %s",g_slave_rx_buff);
...
/*******************************************************************************************************************//**
 * @brief This function demos R_SPI_B_WriteRead() for both Master and Slave simultaneously.
 * @param[IN]   None
...
fsp_err_t spi_write_read(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nSlave R_SPI_B_WriteRead() failed");
        return err;
    }
    ...
    if(FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nMaster R_SPI_B_WriteRead() failed");
        return err;
    }
    ...
    while((SPI_EVENT_TRANSFER_COMPLETE != g_master_event_flag)||(SPI_EVENT_TRANSFER_COMPLETE != g_slave_event_flag))
    {
        ...
        {
            err = FSP_ERR_TIMEOUT;
            APP_ERR_PRINT("\r\nSPI_B module failed in Write/Read operation.\r\n");
            error_print();
            return err;
        }
        else if ((SPI_EVENT_TRANSFER_ABORTED == g_master_event_flag) || (SPI_EVENT_TRANSFER_ABORTED == g_slave_event_flag))
...
void spi_clean_up(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_SPI_B_Close API for master failed **  \r\n");
    }
    ...
    {
        /* SPI Close failure message */
        APP_ERR_PRINT("** R_SPI_B_Close API for slave failed **  \r\n");
    }
}

/*******************************************************************************************************************//**
 * @} (end addtogroup r_spi_b_ep)
 **********************************************************************************************************************/
