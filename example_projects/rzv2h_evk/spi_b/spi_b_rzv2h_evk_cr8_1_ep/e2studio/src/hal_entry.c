        ...
        if (APP_CHECK_DATA)
        {
            ...
            switch (converted_rtt_input)
            {
                case WRITE_AND_READ:
                    ...
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("** SPI_B WRITE AND READ Demo failed **  \r\n");
                        spi_clean_up();
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("\r\n** SPI_B WRITE AND READ Demo Successful**  \r\n");
                    }
                    break;
                case WRITE_READ:
                    err = spi_write_read ();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("\r\n** SPI_B WRITE Demo failed **  \r\n");
                        spi_clean_up();
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("** SPI_B WRITE_READ Demo Successful**  \r\n");
                    }
                    break;
                case EXIT:
                    err = spi_exit_demo();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("** SPI_B EXIT Demo failed ** \r\n");
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
